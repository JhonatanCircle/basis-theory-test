...
window.addEventListener('load', async () => {
  ...
  document.querySelector("#amount").style = style;
...
function displaySuccess() {
  ...
  document.querySelector('#success').innerHTML = 'Test card charged $' + document.getElementById("amount").value + ' with Stripe.';
...
async function submitCard() {
  ...
  try {
    ...
    let tokenDiv = document.getElementById("token_text");
    let content = document.createTextNode(token.id);
    tokenDiv.appendChild(content);
...
async function submitAmount() {
  ...
  try {
    const response = await fetch(`/api/charge`, {
      ...
      body: JSON.stringify({
        card_token_id: document.getElementById("token_text").textContent,
